/**
 * Podcast Image Proxy API Endpoint
 *
 * Serves podcast cover images through a proxy, hiding CloudFront URLs from the client.
 * This is the same pattern used for audio files and episode images.
 *
 * Architecture:
 * - Client requests: /api/images/podcasts/[id]
 * - Server fetches podcast data from database
 ...
 * - Single URL format: /api/images/podcasts/[id]
 * - Proper caching headers
 * - Consistent with episode image and audio patterns
 */
...
export async function GET(
  ...
  try {
    ...
    // 1. Fetch podcast data DIRECTLY from DB (no transformation)
    ...
    // 2. Get RAW image URL from database
    const rawImageUrl = podcast.cover_image;

    if (!rawImageUrl) {
      return NextResponse.json(
        { error: 'Podcast has no cover image' },
        { status: 404 }
      );
    }
    ...
    console.error('Error in podcast image proxy:', error);
    return NextResponse.json(
